  ...
  return (
    <div className="space-y-6">
      <MealPlannerPanel
        ...
        cookingSkillOptions={COOKING_SKILL_OPTIONS}
        applianceOptions={APPLIANCE_OPTIONS}
        ...
        onToggleAppliance={(applianceId) =>
          setSelectedAppliances((current) =>
            current.includes(applianceId)
              ? current.filter((appliance) => appliance !== applianceId)
              : [...current, applianceId]
          )
        }
        recipeFilters={{
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500">
              {profile
                ? `Saved recipes filtered by the same time, skill, appliance, budget, and dietary choices used by the planner.`
                : "Browse suggestions from the saved recipe database. Sign in to apply your preferences."}
            </p>
          </div>
